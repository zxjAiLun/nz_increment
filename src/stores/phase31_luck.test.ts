...
describe('Phase 3.1 — 幸运管线统一', () => {
  ...
  describe('calculateKillDropChances — 掉落规则', () => {
    it('普通怪装备掉率 = base × (1 + multiplierBonus)，封顶 0.95', () => {
      const { equipmentChance } = calculateKillDropChances({ baseEquipmentChance: 0.1, baseDiamondChance: 0.05, luck: 200, isBoss: false })
      const multiplierBonus = Math.min(200 * LUCK_CONFIG.equipmentDropMultiplierPerPoint, LUCK_CONFIG.equipmentDropMultiplierCap)
      expect(equipmentChance).toBeCloseTo(Math.min(0.95, 0.1 * (1 + multiplierBonus)))
    })

    it('Boss 装备基础掉率不受幸运影响', () => {
      const base = 0.2
      const luck = calculateKillDropChances({ baseEquipmentChance: base, baseDiamondChance: 0.05, luck: 800, isBoss: true })
      const noLuck = calculateKillDropChances({ baseEquipmentChance: base, baseDiamondChance: 0.05, luck: 0, isBoss: true })
      expect(luck.equipmentChance).toBe(base)
      expect(noLuck.equipmentChance).toBe(base)
    })

    it('普通怪与 Boss 钻石概率都受幸运提高，封顶 0.95', () => {
      const normal = calculateKillDropChances({ baseEquipmentChance: 0.1, baseDiamondChance: 0.05, luck: 400, isBoss: false })
      const boss = calculateKillDropChances({ baseEquipmentChance: 0.1, baseDiamondChance: 0.5, luck: 400, isBoss: true })
      expect(normal.diamondChance).toBeCloseTo(Math.min(0.95, 0.05 + 400 * LUCK_CONFIG.diamondChancePerPoint))
      expect(boss.diamondChance).toBeCloseTo(Math.min(0.95, 0.5 + 400 * LUCK_CONFIG.diamondChancePerPoint))
    })

    it('幸运不提高单次钻石数量（数量公式不含 luck）', () => {
      // 数量公式 Math.floor(1 + rng()*(isBoss?200:10)) 不含 luck，本函数只返回概率
      expect(calculateKillDropChances({ baseEquipmentChance: 0.1, baseDiamondChance: 0.05, luck: 800, isBoss: false }).diamondChance).toBeLessThanOrEqual(0.95)
    })
  })

  describe('combineIndependentDropChances — talent 独立组合', () => {
    it('与两次独立判定等价：1-(1-a)(1-b)', () => {
      expect(combineIndependentDropChances(0.1, 0.02)).toBeCloseTo(1 - 0.9 * 0.98)
      expect(combineIndependentDropChances(0, 0)).toBe(0)
      expect(combineIndependentDropChances(1, 0.5)).toBeCloseTo(0.975) // 输入先被 clampDropChance 收敛到 0.95
    })
  })

  describe('rollKillDrops — 统一 RNG 顺序 & seeded trace', () => {
    it('相同种子 → 相同掉落（运行时与模拟器共用同一函数，故可对齐）', () => {
      const params = { baseEquipmentChance: 0.5, baseDiamondChance: 0.5, luck: 200, isBoss: false, difficulty: 100, rarityBonus: 0, talentEquipmentDropBonusRate: 0 }
    ...
    it('未掉落时不额外消费数量/装备生成 RNG', () => {
      ...
      const drop = rollKillDrops({ rng: countingRng, baseEquipmentChance: 0.001, baseDiamondChance: 0.001, luck: 0, isBoss: false, difficulty: 100 })
  ...
  describe('damageMonster — 旧怪奖励快照（不私自掉落）', () => {
    beforeEach(() => {
      setActivePinia(createPinia())
    })
    it('击杀返回旧怪的 base 掉率 / Boss 标记 / 金币，且不做掉落判定', () => {
