...
describe('Phase 3.2.1 — 有效 attack/luck 集成（真实 loadGame + 真实公式）', () => {
  ...
  // —— Phase 3.2.2：用真实装备重写（基线与实验组基础 stats 完全相同，仅装备不同）——
  const attackEquipment: Equipment = {
    id: 'phase32-attack-test',
    slot: 'weapon',
    name: 'Phase32 Attack Test',
    rarity: 'common',
    level: 1,
  ...
  const luckEquipment: Equipment = {
    id: 'phase32-luck-test',
    slot: 'weapon',
    name: 'Phase32 Luck Test',
    rarity: 'common',
    level: 1,
    stats: [{ type: 'luck', value: 100, isPercent: false }],
    isLocked: false,
    affixes: [],
    refiningSlots: [],
    refiningLevel: 0,
  ...
  const baseStats = { attack: 10, luck: 10 }

  function loadWithEquipment(
    ...
    const save = craftSave({
      lastOfflineCheckpointAt: NOW - HOUR * 1000,
      player: { offlineEfficiencyBonus: 0, stats, equipment, ...extraPlayer }
    })
    localStorage.setItem(SAVE_KEY, save)
    const store = usePlayerStore()
    store.loadGame()
  ...
  it('真实武器装备 attack → 装备水合进 persistentTotalStats 并增加金币与经验', () => {
    const base = loadWithEquipment({}, baseStats)
    const baseAttack = base.persistentTotalStats.attack
    const baseGold = base.pendingOfflineReward!.gold
    const baseExp = base.pendingOfflineReward!.exp
    const exp = loadWithEquipment({ weapon: attackEquipment }, baseStats)
    // 装备 attack 在 calculateTotalStats 中于养成倍率之后累加为 flat，
    // 故实验组有效攻击 = 基线 + 装备 attack（与养成倍率无关，差值为精确 100）。
    expect(exp.persistentTotalStats.attack).toBe(baseAttack + 100)
    expect(exp.pendingOfflineReward!.gold).toBeGreaterThan(baseGold)
    expect(exp.pendingOfflineReward!.exp).toBeGreaterThan(baseExp)
  })

  it('真实武器装备 luck → 装备水合进 persistentTotalStats 并增加金币、经验不变', () => {
    const base = loadWithEquipment({}, baseStats)
    const baseLuck = base.persistentTotalStats.luck
    const baseGold = base.pendingOfflineReward!.gold
    const baseExp = base.pendingOfflineReward!.exp
    const exp = loadWithEquipment({ weapon: luckEquipment }, baseStats)
    expect(exp.persistentTotalStats.luck).toBe(baseLuck + 100)
    expect(exp.pendingOfflineReward!.gold).toBeGreaterThan(baseGold)
    expect(exp.pendingOfflineReward!.exp).toBe(baseExp)
  })

  it('原始幸运(player.stats.luck) 与 有效幸运(persistentTotalStats.luck) 不同 —— 装备制造差异', () => {
    // 不再依赖 `luck=0 → ||10` 回退，而是用装备 +100 luck 制造有效幸运差异。
    const store = loadWithEquipment({ weapon: luckEquipment }, { attack: 100, luck: 10 })
    const eff = store.persistentTotalStats.luck
    const raw = store.player.stats.luck
    expect(eff).toBe(raw + 100)            // 装备提供 +100 有效幸运
    expect(eff).not.toBe(raw)              // 有效值 ≠ 原始值
...
describe('Phase 3.2.3 — 离线结算持久化可重试（首次写入失败不丢奖励）', () => {
  const NOW = new Date('2026-01-01T00:00:00Z').getTime()
  const BASE_TOTAL = 100
  const OLD_PENDING = { id: 'old', createdAt: 1, elapsedSeconds: 100, creditedSeconds: 100, gold: 10, exp: 5, formulaVersion: 1 as const }
  const NEW_SETTLE = { elapsedSeconds: HOUR, creditedSeconds: HOUR, gold: 100, exp: 50, formulaVersion: 1 as const }
  ...
  // 仅 setItem 抛错的 localStorage；读取委托真实存储（磁盘可读，写入抛错）。
  // 注意：vi.spyOn(localStorage,'setItem') 在本 jsdom 环境下不会拦截 store 模块内的
  // localStorage.setItem 引用，故必须用 stubGlobal 覆盖全局对象本身（沿用 Phase 3.1.1.2 经验）。
  function installThrowingStorage() {
    ...
    const throwing = {
      ...
      setItem: (_k: string, _v: string) => { throw new Error('quota exceeded') }
    }
    vi.stubGlobal('localStorage', throwing)
    return realStorage
  }

  it('首次 save 失败 → 内存保留完整候选奖励 + totalOfflineTime 累加，磁盘不变（不回滚到 null）', () => {
    const realStorage = localStorage
    const diskSave = craftSave({
      lastOfflineCheckpointAt: NOW - HOUR * 1000,
      pendingOfflineReward: OLD_PENDING,
      player: { totalOfflineTime: BASE_TOTAL }
    })
    realStorage.setItem(SAVE_KEY, diskSave)
    installThrowingStorage()

    const store = usePlayerStore()
    vi.spyOn(offlineReward, 'calculateOfflineSettlement').mockReturnValue({ ...NEW_SETTLE })
    store.loadGame() // 写入抛错

    // 内存：old(10/5) + 本次 1h 新结算(100/50) 合并可见；totalOfflineTime 累加。
    expect(store.pendingOfflineReward?.gold).toBe(OLD_PENDING.gold + NEW_SETTLE.gold) // 110
    expect(store.pendingOfflineReward?.exp).toBe(OLD_PENDING.exp + NEW_SETTLE.exp) // 55
    expect(store.player.totalOfflineTime).toBe(BASE_TOTAL + HOUR) // 100 + 3600 = 3700
    // 磁盘：仍是原 save，未推进 checkpoint、未写入候选。
    expect(realStorage.getItem(SAVE_KEY)).toBe(diskSave)

    vi.unstubAllGlobals()
  })

  it('pending=null 的失败恢复：首次保存失败后内存保留 1h 候选，普通 saveGame 成功必须写入候选（不只推进 checkpoint）', () => {
    const realStorage = localStorage
    const diskSave = craftSave({
      lastOfflineCheckpointAt: NOW - HOUR * 1000,
    ...
    vi.spyOn(offlineReward, 'calculateOfflineSettlement').mockReturnValue({ ...NEW_SETTLE })
    store.loadGame() // 写入抛错
    expect(store.pendingOfflineReward?.gold).toBe(NEW_SETTLE.gold) // 内存保留 1h 候选
    expect(realStorage.getItem(SAVE_KEY)).toBe(diskSave) // 磁盘未变

    // 恢复存储并推进 30s，普通自动存档成功。
    vi.unstubAllGlobals()
    vi.advanceTimersByTime(30 * 1000)
    const ok = store.saveGame()
    expect(ok).toBe(true)
    const after = JSON.parse(realStorage.getItem(SAVE_KEY) || '{}')
    expect(after.pendingOfflineReward.gold).toBe(NEW_SETTLE.gold) // 候选已写入
    expect(after.pendingOfflineReward.exp).toBe(NEW_SETTLE.exp)
    expect(after.player.totalOfflineTime).toBe(HOUR)
    expect(after.lastOfflineCheckpointAt).toBe(NOW + 30 * 1000) // checkpoint 随候选一起推进
  })

  it('真实连续链路：失败 → 恢复自动存档 → 再 loadGame 不二次结算 → 合并奖励只领取一次', () => {
    const realStorage = localStorage
    const diskSave = craftSave({
      lastOfflineCheckpointAt: NOW - HOUR * 1000,
      pendingOfflineReward: OLD_PENDING,
    ...
    // 第一步：首次写入失败 → 内存候选可见、磁盘不变。
    store.loadGame()
    expect(store.pendingOfflineReward?.gold).toBe(OLD_PENDING.gold + NEW_SETTLE.gold) // 110
    expect(store.pendingOfflineReward?.exp).toBe(OLD_PENDING.exp + NEW_SETTLE.exp) // 55
    expect(store.player.totalOfflineTime).toBe(BASE_TOTAL + HOUR) // 3700
    const candidateGold = store.pendingOfflineReward!.gold
    const candidateCredited = store.pendingOfflineReward!.creditedSeconds
    const candidateTotal = store.player.totalOfflineTime
    expect(realStorage.getItem(SAVE_KEY)).toBe(diskSave) // 磁盘未变

    // 第二步：模拟 App 自动存档恢复（真实 storage + 推进 30s）。
    vi.unstubAllGlobals()
    vi.advanceTimersByTime(30 * 1000)
    const ok = store.saveGame()
    expect(ok).toBe(true)
    const afterSave = JSON.parse(realStorage.getItem(SAVE_KEY) || '{}')
    expect(afterSave.pendingOfflineReward.gold).toBe(OLD_PENDING.gold + NEW_SETTLE.gold) // 合并一次，恰好 110
    expect(afterSave.pendingOfflineReward.exp).toBe(OLD_PENDING.exp + NEW_SETTLE.exp) // 55
    expect(afterSave.player.totalOfflineTime).toBe(BASE_TOTAL + HOUR) // 3700
    expect(afterSave.lastOfflineCheckpointAt).toBe(NOW + 30 * 1000)

    // 第三步：相同时间再次 loadGame → 不应二次结算。
    store.loadGame()
    expect(store.pendingOfflineReward?.gold).toBe(candidateGold) // 不增加
    expect(store.pendingOfflineReward?.creditedSeconds).toBe(candidateCredited) // 不增加
    expect(store.player.totalOfflineTime).toBe(candidateTotal) // 不增加

    // 第四步：领取两次 → 第一次发放合并奖励并清 pending，第二次返回 null。
    const g0 = store.player.gold
    const first = store.claimOfflineReward()
    expect(first?.gold).toBe(candidateGold)
    expect(first?.creditedSeconds).toBe(candidateCredited)
    expect(store.player.gold).toBe(g0 + candidateGold) // 恰好增加一次
    expect(store.pendingOfflineReward).toBeNull()
    const second = store.claimOfflineReward()
    expect(second).toBeNull() // 不再发放
    expect(store.player.gold).toBe(g0 + candidateGold)
    const diskAfterClaim = JSON.parse(realStorage.getItem(SAVE_KEY) || '{}')
    expect(diskAfterClaim.pendingOfflineReward).toBeNull()
  })

  it('正常保存后再次 loadGame 不二次结算', () => {
    localStorage.setItem(SAVE_KEY, craftSave({
      lastOfflineCheckpointAt: NOW - HOUR * 1000,
      pendingOfflineReward: null
    }))
    const store = usePlayerStore()
    store.loadGame()
    const first = store.pendingOfflineReward!
    expect(first.creditedSeconds).toBe(3600) // 1h 区间
    const firstTotal = store.player.totalOfflineTime
    expect(firstTotal).toBe(3600)

    // 再 loadGame（checkpoint 已成功推进到 NOW）→ 不应再产生结算。
    store.loadGame()
    const second = store.pendingOfflineReward!
    expect(second.creditedSeconds).toBe(3600) // 仍是原 1h，未叠加
    expect(store.player.totalOfflineTime).toBe(firstTotal) // 未增加
  })

  it('recordLogout 不在写入成功前推进内存 checkpoint（LAST_FLOOR_KEY 失败也不提前推进）', () => {
    const store = usePlayerStore()
    // 模拟已加载：把内存 checkpoint 设为一个明确旧值（区别于 now），用于检测是否提前推进。
    const oldCp = NOW - 10 * HOUR * 1000
    ...
    store.recordLogout() // setItem 抛错 → saveGame 失败 → checkpoint 不推进
    expect(store.lastOfflineCheckpointAt).toBe(oldCp)
    vi.unstubAllGlobals()
  })
})

describe('Phase 3.2.2 — createdAt 规范化（复用正向时间戳语义）', () => {
  it('非法 createdAt 一律生成正有限时间', () => {
    const illegalValues: unknown[] = [
      null, undefined, '', '   ', 'null', 'NaN', 'Infinity', 0, -1, NaN, Infinity
    ]
    for (const bad of illegalValues) {
      const norm = normalizePendingOfflineReward({ gold: 5, exp: 2, createdAt: bad })
      expect(norm).not.toBeNull()
      expect(Number.isFinite(norm!.createdAt)).toBe(true)
      expect(norm!.createdAt).toBeGreaterThan(0)
    }
  })

  it('合法正时间戳保持不变', () => {
    const t = 1_700_000_000_000
    const norm = normalizePendingOfflineReward({ gold: 5, exp: 2, createdAt: t })
    expect(norm!.createdAt).toBe(t)
  })

  it('旧 {gold,exp} 迁移且 createdAt 非法时补合法值，金额不变', () => {
    const norm = normalizePendingOfflineReward({ gold: 7, exp: 3, createdAt: 'NaN' })
    expect(norm!.gold).toBe(7)
    expect(norm!.exp).toBe(3)
    expect(norm!.createdAt).toBeGreaterThan(0)
  })
})
