...
describe('Phase 3.2.2 — 离线结算持久化事务（save 失败回滚，不双发）', () => {
  const NOW = new Date('2026-01-01T00:00:00Z').getTime()
  ...
  it('localStorage.setItem 抛错 → 新结算未提交、字段回滚、磁盘不变', () => {
    // 预备：磁盘 checkpoint = NOW-1h，无 pending（用真实 storage 写入）。
    ...
    // 用 stubGlobal 替换全局 localStorage：仅 setItem 抛错，读取委托真实 storage（磁盘可读）。
    // 注意：vi.spyOn(localStorage,'setItem') 在本 jsdom 环境下不会拦截 store 模块内的
    // localStorage.setItem 引用，故必须用 stubGlobal 覆盖全局对象本身。
    const throwingStorage = {
    ...
    const store = usePlayerStore()
    const prevTotalOffline = store.player.totalOfflineTime
    store.loadGame() // 写入会抛错

    // 新区间未提交：内存 pending 为空、totalOfflineTime 未增加（回滚到入口状态）。
    expect(store.pendingOfflineReward).toBeNull()
    expect(store.player.totalOfflineTime).toBe(prevTotalOffline)

    // 磁盘内容保持不变（仍是原 save，checkpoint = NOW-1h，无 pending）。
    expect(realStorage.getItem(SAVE_KEY)).toBe(diskSave)

    vi.unstubAllGlobals()
  })

  it('恢复存储后该 1h 区间仅结算一次；再次 loadGame 不二次结算', () => {
    // 真实 storage 预备磁盘 checkpoint = NOW-1h，pending 为 null。
    localStorage.setItem(SAVE_KEY, craftSave({
      lastOfflineCheckpointAt: NOW - HOUR * 1000,
      pendingOfflineReward: null
    }))
