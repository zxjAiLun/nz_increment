...
当前阶段不继续横向加系统，而是把前 30 分钟打磨成清晰主线：

| 时间/难度 | 玩家体验目标 | 开放系统 |
|---|---|---|
| 0-5 分钟 / 0-10 难度 | 明白攻击、掉落、装备 | 主线战斗、装备、属性强化 |
| 5-10 分钟 / 首个 Boss | 感到第一次生存压力 | Boss 反馈、生命/防御强化 |
| 10-20 分钟 / 10 难度后 | 获得长期目标 | 抽卡、修炼/命座 |
| 20-30 分钟 / 20-30 难度 | 开始形成构筑 | 技能、套装/加成、战报解释、装备比较 |

主界面的核心信息优先级固定为：下一目标、当前瓶颈、推荐行动、预计收益。系统解锁必须回答它解决了哪个痛点、给了什么选择、是否改变构筑。

## 0.1 五大流派闭环

| 流派 | 核心属性 | 代表装备词条 | 代表套装 | 代表技能 | 克制 Boss | 弱点 Boss | UI 标签 |
|---|---|---|---|---|---|---|---|
| 暴击爆发流 | 攻击、暴击、暴伤 | `attack` / `critRate` / `critDamage` | 勇者、暴君 | 暴击强化、血性狂暴、流星陨落 | 狂暴、汲血 | 高护甲、护盾 | 大数字、短时爆发 |
| 吸血坦克流 | 生命、防御、吸血 | `maxHp` / `defense` / `lifesteal` / `damageReduction` | 守护者、血誓守护 | 防御姿态、生命汲取、神圣祝福 | 狂暴、护盾 | 汲血、高护甲 | 站得住、回血 |
| 破甲真伤流 | 穿透、真伤、虚空伤害 | `penetration` / `trueDamage` / `voidDamage` | 虚空 | 穿甲打击、虚空之箭、贯穿之箭 | 高护甲 | 高闪避、狂暴 | 无视护甲、高防克星 |
| 极速技能流 | 速度、技能伤害、冷却 | `speed` / `skillDamageBonus` / `cooldownReduction` / `attackSpeed` | 疾风 | 疾风步、旋风斩、连锁闪电 | 护盾、高闪避 | 高护甲、汲血 | 高频出手、多段技能 |
| 幸运寻宝流 | 幸运、速度、命中 | `luck` / `speed` / `accuracy` | 寻宝者 | 连刺、疾风步 | 资源循环 | 狂暴、高护甲、护盾 | 掉落更多、战斗偏弱 |

生命偷取从装备池进入，装备舒适软上限为 10%，最终上限为 15%。幸运寻宝流必须保持“收益高但战斗弱”的取舍，卡 Boss 时应切换输出或生存流派。
...
默认输出 `reports/balance-report.md`，列包含：难度、构筑、场景、护栏状态、胜率、平均 TTK/TTL、死亡率、金币/分钟、装备/分钟、主要失败原因、推荐关注属性，并在附表保留怪物属性与玩家 DPS。`balance-report` 用于观察，不因护栏失败而阻断。
...
当前护栏采用温和阈值，优先发现明显异常：普通怪 TTK 过长、Boss 长期低胜率、高防 Boss 未要求破甲、高闪避 Boss 未要求命中、幸运流同时成为最高战斗收益。`recommendedStat` 是规则映射，不是自动搜索最优加点。
